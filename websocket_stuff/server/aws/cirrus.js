...
playerServer.on('connection', function (ws, req) {
	...
	ws.on('message', function (msg) {
		...
		// ----------------------------- CHECK FOR SCRATCH MESSAGE EVERYTIME ------------------------------------------
		...
		if (test == 'scratch'){     // if its a message from scratch then display on console
			console.log('SCRATCH COMS:' + msg2);
			playerServer.clients.forEach((client) => {
			client.send(msg);
 			});
		}
		// ------------------------------- END OF SCRATCH, CONTINUE NORMAL OPERATIONS --------------------------------
		else{
			try {
				msg = JSON.parse(msg);
			} catch (err) {
			...
			if (msg.type == 'offer') {
				...
				// TIME TO CHECK SCRATCH JSON ----- < < < < < < < < < < < < < < < < < < < < < < -----------------
				// if (msg.scratch == true){
				// 	console.log('SCRATCH COMS:' + msg.content);
				// 	playerServer.clients.forEach((client) => {
	...
	function initShutDown() {
		console.log("Player disconnected. Initiating shutdown...");
		awsInstanceMetadata.fetch('instance-id')
		.then((instanceId) => {
		console.log("Our instanceId is " + instanceId);
		...
		//console.log(JSON.stringify(paramsStop));
		
		ec2.stopInstances(paramsStop, function(err, dataStop) {
			//console.log(err, dataStop);
			if (err) {
