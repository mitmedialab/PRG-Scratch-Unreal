...
// modified for scratch blocks. Replace cirrus.js with this file in AWS machine. Rename "cirrus_test.js" to "cirrus.js"
...
// -------------- CLASSIC WAY OF SETTING UP WEBSOCKET SERVER ---------------- DOESN"T WORK REMOTELY --------------
let WebSocket = require('ws');

// let scratchCom = new WebSocket.Server({port:8889});

// scratchCom.on('connection', function(scratchClient){
// 	console.log("Scratch Websocket Connection Established!");
// 	scratchClient.onmessage = function(e){
// 		let msg = e.data;
// 		console.log(msg);
// 		let test = msg.slice(0, 7)

// 		if (test == 'scratch'){     // if its a message from scratch then send it to all clients
// 			scratchCom.clients.forEach((client) => {
...
// ----------------- HIJACK THIS SERVER CONNECTION FOR SCRATCH ------------------------
...
streamerServer.on('connection', function (ws, req) {
	...
	ws.on('message', function onStreamerMessage(msg) {
		// console.logColor(logging.Blue, `<- Streamer: ${msg}`); MAKE SURE TO RESTORE THIS LINE
		// BEGIN CHECKING INPUTS FOR SCRATCH
		// let msg2 = msg; 	// copying just incase
		// let test = msg2.slice(0, 7)

		// if (test == 'scratch'){     // if its a message from scratch then display on console
		// 	console.log('scratch received:' + test);
